...
- **Daily scan:** runs automatically via the service's Oban cron (`{"0 13 * * *", MarketplaceBot.Jobs.DailyScan}`, 13:00 UTC ≈ 8am Central). Manual run: `mix run -e 'MarketplaceBot.Jobs.DailyScan.run([])'` (note: `mix run` does not bind the port, so it coexists with the service).
